# 📈 ApexPulse: Financial Performance & Regional Intelligence Dashboard
...
• Facilitates drill-down analysis into product categories, regions, and individual states (e.g., Andaman and Nicobar Islands, Andhra Pradesh, Arunachal Pradesh, and Assam).
• Visualizes forecasting trends with upper and lower confidence bounds.
• Breaks down the exact financial levers impacting bottom-line revenue changes.

---

### Walkthrough of Key Visuals 
• **Global KPIs & Time Slicers (Top Left):** 🎛️ Interactive year selectors (`FY22` to `FY26`) and region filters let you instantly slice and dice high-level KPI cards displaying Total Revenue (362.25bn), Profit Margin (14.59%), and Total Volume (1.45bn Liters).  
• **Advanced Trend & Prediction (Top Right):** 🔍 Features a Product Category drill-down bar and a Forecast vs Actual Trend line chart tracking performance across months with confidence boundaries.  
• **Regional Revenue Density (Bottom Left - Map):** 🗺️ An interactive geographic choropleth map of India segmented by East, North, South, and West zones to dynamically visualize spatial revenue concentration.  
• **Category Profitability and Volume (Bottom Center - Treemap):** 📦 A clickable hierarchical treemap displaying revenue contributions and volume distributions across product categories by region.  
• **YoY Growth by State (Bottom Right - Matrix Table):** 📋 A scrollable, detailed tabular matrix breaking down Total Revenue, Profit Margin %, and Total Volume (Litres) per state alongside grand totals.  
• **Revenue Change Drivers (Bottom Right - Waterfall Chart):** 🌊 A financial waterfall chart explaining variances from Starting Revenue to Ending Revenue through interactive drivers like Volume Growth, Price Realization, Market Share Gains, Raw Material Price Changes, and Discounts.
...
• **Predictive Planning:** Forecast trend models assist inventory and supply chain teams in anticipating future volume demands (measured in liters) across seasonal cycles.
